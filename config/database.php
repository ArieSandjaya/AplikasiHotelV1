...
class medoo
{
    ...
    protected $database_name = "db_hotel";
    ...
    public function __construct($options = null)
    {
        try {
            ...
            if (
                in_array(
                    $type,
                    explode(" ", "mariadb mysql pgsql sybase mssql")
                ) &&
                $this->charset
            ) {
    ...
    protected function column_push($columns)
    {
        ...
        foreach ($columns as $key => $value) {
            ...
            if (isset($match[1], $match[2])) {
                array_push(
                    $stack,
                    $this->column_quote($match[1]) .
                        " AS " .
                        $this->column_quote($match[2])
                );
            } else {
    ...
    protected function inner_conjunct($data, $conjunctor, $outer_conjunctor)
    {
        ...
        foreach ($data as $value) {
            $haystack[] = "(" . $this->data_implode($conjunctor, $value) . ")";
        }
    ...
    protected function data_implode(
        $data,
        $conjunctor,
        $outer_conjunctor = null
    ) {
        $wheres = [];
            ...
            if (
                ...
                if (isset($match[4])) {
                    ...
                    switch ($type) {
                        ...
                        case "string":
                            $wheres[] =
                                $column . " = " . $this->fn_quote($key, $value);
                            break;
                    }
                }
            }
    ...
    protected function where_clause($where)
    {
        ...
        if (is_array($where)) {
            ...
            if ($single_condition != []) {
                $where_clause =
                    " WHERE " . $this->data_implode($single_condition, "");
            }
            if (!empty($where_AND)) {
                $value = array_values($where_AND);
                $where_clause =
                    " WHERE " . $this->data_implode($where[$value[0]], " AND");
            }
            if (!empty($where_OR)) {
                $value = array_values($where_OR);
                $where_clause =
                    " WHERE " . $this->data_implode($where[$value[0]], " OR");
            }
            ...
            if (isset($where["GROUP"])) {
                $where_clause .=
                    " GROUP BY " . $this->column_quote($where["GROUP"]);
                if (isset($where["HAVING"])) {
                    $where_clause .=
                        " HAVING " .
                        $this->data_implode($where["HAVING"], " AND");
                }
            }
    ...
    {
        $query = $this->query(
            $this->select_context($table, $join, $columns, $where)
        );
        return $query
            ? $query->fetchAll(
    ...
    {
        return $this->exec(
            'DELETE FROM "' . $table . '"' . $this->where_clause($where)
        );
    }
    public function replace(
        ...
        if (is_array($columns)) {
            ...
            foreach ($columns as $column => $replacements) {
                foreach (
                    $replacements
                    as $replace_search => $replace_replacement
                ) {
                    $replace_query[] =
                        $column .
                        " = REPLACE(" .
                        $this->column_quote($column) .
                        ", " .
                        $this->quote($replace_search) .
                        ", " .
    ...
    {
        $query = $this->query(
            $this->select_context($table, $join, $column, $where) . " LIMIT 1"
        );
        if ($query) {
    ...
    {
        $query = $this->query(
            $this->select_context($table, $join, $column, $where, "COUNT")
        );
    ...
    {
        $query = $this->query(
            $this->select_context($table, $join, $column, $where, "MAX")
        );
        if ($query) {
    ...
    {
        $query = $this->query(
            $this->select_context($table, $join, $column, $where, "MIN")
        );
        if ($query) {
    ...
    {
        $query = $this->query(
            $this->select_context($table, $join, $column, $where, "AVG")
        );
        return $query ? 0 + $query->fetchColumn() : false;
    }
    ...
    {
        $query = $this->query(
            $this->select_context($table, $join, $column, $where, "SUM")
        );
        return $query ? 0 + $query->fetchColumn() : false;
    }
